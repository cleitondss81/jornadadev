...
TCC/
├── Dados-e-Dicionario/
│   ├── sx2990.csv         ← Dicionário de Tabelas (SX2)
│   ├── sx3990.csv         ← Dicionário de Campos (SX3)
│   ├── six990.csv         ← Dicionário de Índices (SIX)
│   ├── sx7990.csv         ← Dicionário de Gatilhos (SX7)
│   ├── sxb990.csv         ← Dicionário de Consultas / F3 (SXB)
│   ├── zz1990.csv         ← Tabela ZZ1 (dados de teste)
│   ├── zz2990.csv         ← Tabela ZZ2 (dados de teste)
│   ├── sa2990.dbf         ← Fornecedores de teste (SA2)
│   ├── sb1990.dbf         ← Produtos de teste (SB1)
│   ├── sigacom.xnu        ← Menu do módulo de Compras
│   └── converte-dicionario.prg  ← Utilitário Harbour que exporta o dicionário (.dbf → .csv)
...
├── TCC.PRJ               ← Projeto da IDE (lista dos fontes)
├── TCC-Documentacao.docx ← Documentação completa com prints das telas
└── README.md             ← Este documento
```

> O dicionário e as tabelas ZZ1/ZZ2 foram exportados do ambiente para **.csv** (fáceis de ler e leves), usando o utilitário `converte-dicionario.prg` (compilado em Harbour). Os fornecedores (SA2) e produtos (SB1) de teste vão como **.dbf**. O conteúdo também está descrito, campo a campo, nas seções 3 a 8 deste README.
...
Os gatilhos estão no dicionário de gatilhos (`sx7990.csv`).
...
### Passo 1 — Dicionário
O dicionário está nos arquivos **.csv** da pasta `Dados-e-Dicionario/` (SX2, SX3, SIX, SX7, SXB) e descrito nas seções 3 a 8 deste README. Para reproduzir em outro ambiente, cadastre no **SIGACFG > Base de Dados > Dicionário (CFGX031)** as tabelas **ZZ1** e **ZZ2**:
...
|---|---|---|
| Dicionário (tabelas, índices, SX3) | 20% | `Dados-e-Dicionario/` (.csv) + seções 3 a 8 |
...
| Gatilhos automáticos | 10% | `sx7990.csv` (seção 5) |
